...
public class Creation implements Saveble {
    ...
    private HBox firstButtonrow = SceneBuilder.makeButtonrow();
    private HBox secondButtonrow = SceneBuilder.makeButtonrow();
    private HBox thirdButtonrow = SceneBuilder.makeButtonrow();
    ...
    public void makeScene(int stage) {
        ...
        if (builder) {
            ...
            scene = new Scene(SceneBuilder.buildBuilderScene(firstButtonrow, secondButtonrow, thirdButtonrow, gametext));
        } else {

            if (stage == 1) {
                {
                    firstButtonrow = SceneBuilder.makeButtonrow();
                    secondButtonrow = SceneBuilder.makeButtonrow();
                    thirdButtonrow = SceneBuilder.makeButtonrow();
                ...
                name.prefWidthProperty().bind(Bindings.divide(firstButtonrow.widthProperty(), 7));
                name.setTextFill(Paint.valueOf("ffffff"));
                name.setText("Name :");

                TextField eingabe = new TextField();
                eingabe.prefWidthProperty().bind(Bindings.divide(firstButtonrow.widthProperty(), 7));

                Button accept = SceneBuilder.makeButton(firstButtonrow);
                ...
                firstButtonrow.getChildren().addAll(name, eingabe, accept);
            }
             ...
             */
                ...
                Button cont = SceneBuilder.makeButton(firstButtonrow);
                cont.setText("Continue");
                cont.setOnMouseClicked(event -> Main.getStage().setScene(getScene(3)));

                firstButtonrow.getChildren().addAll(cont);
            }
            ...
            scene = new Scene(SceneBuilder.buildGameScene(firstButtonrow, secondButtonrow, thirdButtonrow, gametext));
        }
    }
    ...
    private TextFlow errormessage(int stage) {
        ...
        Text a = SceneBuilder.makeText("Error in Creation; Stage: " + stage);
    ...
    private TextFlow createBuilderStageoneflow() {
        ...
        Text a = SceneBuilder.makeText("You arrived on Shorin with the goal of creating your own home. You travel to the edge of the Kaguya Forest," +
    ...
    private void createBuilderStageoneButtons() {
        ...
        Label name = SceneBuilder.makeLabel(firstButtonrow, "Name :");
        TextField eingabe = SceneBuilder.createTextfield(firstButtonrow);
        Button accept = SceneBuilder.makeButton(firstButtonrow, "Accept");
